...
"#,
    ...
    fs::write(
        ...
        r#"{
  inputs = {
    py-harbor.url = "git+https://github.com/caniko/py-harbor.git";
...
fn cross_print_emits_multi_target_flake_with_contract() {
    ...
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(stdout.contains("--- flake.nix"));
    assert!(stdout.contains("rs-harbor.url = \"git+https://github.com/caniko/rs-harbor.git?ref=trunk&rev=b40cd4c4fdf6133962f67bd68a48bfd5d554d47f\";"));
