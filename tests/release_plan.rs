...
fn release_plan_dry_run_package_fails_fast_in_publish_order() {
    ...
    let stdout = String::from_utf8(output.stdout).unwrap();
    // Archive-construction honesty: `--no-verify` proves the `.crate` file
    // builds, not that packaged contents verify or publish.
    assert!(stdout.contains("archive construction only"));
    assert!(stdout.contains("a 0.1.0"));
