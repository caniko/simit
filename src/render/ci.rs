...
pub const LEGACY_GENERATED_WORKFLOW_MARKER: &str =
    "Generated by simit. Manual edits will be reported as ci=drift.";
pub const INSTALL_NIX_ACTION_URL: &str = "https://github.com/cachix/install-nix-action";
pub const SIMIT_SOURCE_URL: &str = "https://simit.rs";
pub const SIMIT_RELEASE_BUILD_TYPE: &str = "https://simit.rs/release";
pub const SIMIT_RELEASE_ARTIFACTS_BUILD_TYPE: &str = "https://simit.rs/release-artifacts";

/// Whether a workflow file carries simit's generated-file marker. The legacy
...
fn push_release_integrity_steps(workflow: &mut String, platform: Platform) {
    ...
    workflow.push_str("            jq -n --arg builder_id \"$builder_id\" --arg git_sha \"$git_sha\" --arg workflow_sha \"$workflow_sha\" --arg flake_lock_sha \"$flake_lock_sha\" --arg artifact \"$(basename \"$file\")\" --arg artifact_sha \"$artifact_sha\" '{buildDefinition:{buildType:\"");
    workflow.push_str(SIMIT_RELEASE_ARTIFACTS_BUILD_TYPE);
    workflow.push_str("\",externalParameters:{artifact:$artifact,artifactDigest:{sha256:$artifact_sha}},internalParameters:{},resolvedDependencies:[{uri:\"git\",digest:{gitCommit:$git_sha}},{uri:\"release-artifacts workflow\",digest:{sha256:$workflow_sha}},{uri:\"flake.lock\",digest:{sha256:$flake_lock_sha}}]},runDetails:{builder:{id:$builder_id}}}' > \"$predicate\"\n");
