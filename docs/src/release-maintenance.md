...
Projects that publish release artifacts must also commit the release trust
roots and configure signing secrets. See
[Release Integrity](getting-started/release-integrity.md) for the
`keys/maintainers.gpg`, `keys/minisign.pub`, minisign, cosign, and SLSA
requirements generated by `simit init-ci --with-artifacts`.
